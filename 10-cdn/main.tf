resource "aws_cloudfront_distribution" "sunil-devops" {
  enabled             = true
  ...
  records = [
    {
      ...
      type    = "A"
      alias   = {
        name    = aws_cloudfront_distribution.sunil-devops.domain_name
        zone_id = aws_cloudfront_distribution.sunil-devops.hosted_zone_id
      }
    }
  ]
}